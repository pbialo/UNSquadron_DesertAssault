# Source File Name: 3_0.py
# Author's Name: Paul Bialo
# Date Last Modified: August 16, 2012
...
#                   Version 2.1 - Adding transition screen between stages
#                               - Adjusting the large turret class (and it's shot class) for stage 4 implementation
#                               - Some fixes to the small turret class
#                   Version 2.2 - Adding a splash screen
#                               - Change to scoring system to promote efficient use of weapons
#                   Version 3.0 - Some minor touch-ups for launch.
...
class HeliShot(pygame.sprite.Sprite):
    def __init__(self, heliX, heliY, jetX, jetY):
        ...
        # These will store the coordinates of the shot to avoid an issue with incrementing the 
...
# Implemented in Part B. A stronger and heavier damage turret
class LargeTurret(pygame.sprite.Sprite):
    ...
    # This is used to determine which image is used for the turret
    # Based on relative position of jet
    def targeting(self, jetX, jetY):
        self.targetingX, self.targetingY = enemyProjectileVector(self.rect.centerx, self.rect.centery, jetX, jetY)
        self.theta = radianToDegree(math.atan2(self.targetingY, self.targetingX))
        if self.theta <= 190:
    ...
    def getTurretX(self):
        ...
        if self.frame == 1:
            return (self.rect.centerx - 34)
        if self.frame == 2:
            return (self.rect.centerx - 26)
        if self.frame == 3:
            return (self.rect.centerx - 19)
        if self.frame == 4:
            return (self.rect.centerx)
        if self.frame == 5:
            return (self.rect.centerx + 19)
    ...
    def getTurretY(self):
        ...
        if self.frame == 2:
            return (self.rect.centery - 20)
        if self.frame == 3:
            return (self.rect.centery - 26)
        if self.frame == 4:
            return (self.rect.centery - 38)
        if self.frame == 5:
            return (self.rect.centery - 26)
        if self.frame == 6:
            return (self.rect.centery - 20)
...
# Fired from the large turret, the angle is incremental
class LargeTurretShot(pygame.sprite.Sprite):
    def __init__(self, turretX, turretY, jetX, jetY):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("img/enemies/largeshot4_0.gif")
        self.image = self.image.convert()
        self.rect = self.image.get_rect()
        
        self.turretX = turretX
        self.turretY = turretY
        self.shotX = turretX
        self.shotY = turretX      
        ...
        self.rect.center = (turretX, turretY)

        # Using our enemyProjectileVector function 
        self.targetingX, self.targetingY = enemyProjectileVector(turretX, turretY, jetX, jetY)
        self.theta = radianToDegree(math.atan2(self.targetingY, self.targetingX))
    ...
    # Depending on the relative position of the plane, the image and direction fired will vary
    def loadPicsAngle(self):
        if self.theta <= 190 and self.theta > 160:
            for i in range(2):
                imgName = "img/enemies/largeshot0_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 180 (straight left)
                self.dx = -13
                self.dy = 0
        if self.theta <= 160 and self.theta > 140:
            for i in range(2):
                imgName = "img/enemies/largeshot1_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 155
                self.dx = -12.06
                self.dy = -4.22
        if self.theta <= 140 and self.theta > 120:
            for i in range(2):
                imgName = "img/enemies/largeshot2_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 130
                self.dx = -9.43
                self.dy = -7.66                    
        if self.theta <= 120 and self.theta > 100:
            for i in range(2):
                imgName = "img/enemies/largeshot3_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 105
                self.dx = -5.59
                self.dy = -9.66                   
        if self.theta <= 100 and self.theta > 80:
            for i in range(2):
                imgName = "img/enemies/largeshot4_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 90 (straight up)
                self.dx = -3
                self.dy = -10                   
        if self.theta <= 80 and self.theta > 60:
            for i in range(2):
                imgName = "img/enemies/largeshot5_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 75
                self.dx = -1.59
                self.dy = -9.66                   
        if self.theta <= 60 and self.theta > 40:
            for i in range(2):
                imgName = "img/enemies/largeshot6_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 50
                self.dx = 3.43
                self.dy = -7.66                    
        if self.theta <= 40 and self.theta > 10:
            for i in range(2):
                imgName = "img/enemies/largeshot7_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg)
                # 25
        ...
        if self.theta <= 10:
            for i in range(2):
                imgName = "img/enemies/largeshot8_%d.gif" % i
                tmpImg = pygame.image.load(imgName)
                tmpImg.convert()
                self.img.append(tmpImg) 
                # 0 (straight right)
...
class Stage2Clear (pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("img/gui/stage2clear.gif")
        self.image = self.image.convert()
        self.rect = self.image.get_rect() 
        self.rect.center = (320, 240)
        tranColor = self.image.get_at((1, 1))
...
class Stage3Clear (pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("img/gui/stage3clear.gif")
        self.image = self.image.convert()
        self.rect = self.image.get_rect() 
        self.rect.center = (320, 240)
        tranColor = self.image.get_at((1, 1))
...
class Stage4Clear (pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("img/gui/stage4clear.gif")
        self.image = self.image.convert()
        self.rect = self.image.get_rect() 
        self.rect.center = (320, 240)
        tranColor = self.image.get_at((1, 1))
...
# Image used for the splash screen 
class SplashScreenImage(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("img/gui/splash0.gif")
        self.image = self.image.convert()
        self.rect = self.image.get_rect()
        self.img = []
        self.loadPics()
    ...
    def loadPics(self):
        for i in range(11):
            imgName = "img/gui/splash%d.gif" % i
            tmpImg = pygame.image.load(imgName)
            tmpImg.convert()
            self.img.append(tmpImg)
...
# The splash screen function, will disappear after 100 ticks (3 1/3 seconds)   
def splashScreen():
    splashScreenImage = SplashScreenImage()
    pygame.display.set_caption("UN Squadron: Desert Assault")
    splashScreenSprites = pygame.sprite.Group(splashScreenImage)
    ...
    while keepGoing:
        clock.tick(30)
        if timer.returnTick() == 100:
            splashDone = True
            keepGoing = False
                 
        splashScreenSprites.draw(screen)        
        splashScreenSprites.update()
...
def game():
    ...
    largeTurretShotSprites = pygame.sprite.Group()
    ...
    while keepGoing:
        ...
        # Collisions for the large turret
        for largeTurret in largeTurretSprites:
            for bullet in bulletSprites:
                largeTurretCollisionCheck = pygame.sprite.collide_mask(bullet, largeTurret)
                if largeTurretCollisionCheck:
                    largeTurret.hp -= bullet.damage
                    bullet.kill()
                    if largeTurret.hp <= 0:
                        medExplosion = MedExplosion(largeTurret.getPositionX(), largeTurret.getPositionY())
                        explosionSprites.add(medExplosion)
                        scoreboard.score += 400
                        largeTurret.kill()
                        jet.sndMedExplosion.play()
            for bomb in bombSprites:
                largeTurretCollisionCheck = pygame.sprite.collide_mask(bomb, largeTurret)
                if largeTurretCollisionCheck:
                    largeTurret.hp -= bomb.damage
                    bombExplosion = BombExplosion(largeTurret.getPositionX(), largeTurret.getPositionY())
                    explosionSprites.add(bombExplosion)
                    jet.sndBomb.play()
                    bomb.kill()
                    if largeTurret.hp <= 0:
                        ...
                        jet.sndMedExplosion.play()
            for cluster in clusterSprites:
                largeTurretCollisionCheck = pygame.sprite.collide_mask(cluster, largeTurret)
                if largeTurretCollisionCheck:
                    medExplosion = MedExplosion(largeTurret.getPositionX(), largeTurret.getPositionY())
                    explosionSprites.add(medExplosion)
                    scoreboard.score += 400
        ...
        # Large turrets will shoot 3 burst of 4
        for largeTurret in largeTurretSprites:
            largeTurret.targeting(jet.getPositionX(), jet.getPositionY())
            for n in range(4):
                if largeTurret.counter == 5 * n + 20:
                    largeTurretShot = LargeTurretShot(largeTurret.getTurretX(), largeTurret.getTurretY(), jet.getPositionX(), jet.getPositionY())
                    largeTurretShotSprites.add(largeTurretShot)
            for n in range(4):
                if largeTurret.counter == 5 * n + 120:
                    largeTurretShot = LargeTurretShot(largeTurret.getTurretX(), largeTurret.getTurretY(), jet.getPositionX(), jet.getPositionY())
                    largeTurretShotSprites.add(largeTurretShot) 
            for n in range(4):
                if largeTurret.counter == 5 * n + 220:
                    largeTurretShot = LargeTurretShot(largeTurret.getTurretX(), largeTurret.getTurretY(), jet.getPositionX(), jet.getPositionY())
                    largeTurretShotSprites.add(largeTurretShot) 
        ...
        # Checking for collisions with the player jet, lose health, go invul temporarily, and play warning sound
        for heliShot in heliShotSprites:
        ...
        for flameTurret in flameTurretSprites:
            ...
            if flameTurretJetCollisionCheck and healthBar.invul == 0:
                ...
                jet.sndWarning.play()
        for largeTurret in largeTurretSprites:
            largeTurretJetCollisionCheck = pygame.sprite.collide_mask(largeTurret, jet)
            if largeTurretJetCollisionCheck and healthBar.invul == 0:
                healthBar.invul = 1
                healthBar.health -= 1
                healthBar.count = 0 
                jet.sndWarning.play()
        for largeTurretShot in largeTurretShotSprites:
            largeTurretShotJetCollisionCheck = pygame.sprite.collide_mask(largeTurretShot, jet)
            if largeTurretShotJetCollisionCheck and healthBar.invul == 0:
                healthBar.invul = 1
                healthBar.health -= 1
                healthBar.count = 0 
                largeTurretShot.kill()
                jet.sndWarning.play()
        ...
        # Small breaks between stages, 1 cluster is restocked to a max of 2 during each transition
        if timer.returnTick() == 1100:
            clusterGUI.qty += 1
            if clusterGUI.qty > 2:
                clusterGUI.qty = 2
            stage1Clear = Stage1Clear()
            transitionSprites.add(stage1Clear)
            
        if timer.returnTick() == 1200:
            stage1Clear.kill()
            
        if timer.returnTick() == 2450:
            clusterGUI.qty += 1
            if clusterGUI.qty > 2:
                clusterGUI.qty = 2
            stage2Clear = Stage2Clear()
            transitionSprites.add(stage2Clear)
            
        if timer.returnTick() == 2600:
            stage2Clear.kill()

        if timer.returnTick() == 3450:
            clusterGUI.qty += 1
            if clusterGUI.qty > 2:
                clusterGUI.qty = 2
            stage3Clear = Stage3Clear()
            transitionSprites.add(stage3Clear)
            
        if timer.returnTick() == 3600:
            stage3Clear.kill()

        if timer.returnTick() == 4800:
            clusterGUI.qty += 1
            if clusterGUI.qty > 2:
                clusterGUI.qty = 2
            stage4Clear = Stage4Clear()
            transitionSprites.add(stage4Clear)
            
        if timer.returnTick() == 5000:
            stage4Clear.kill()
            
        # Upon reaching the end alive, add a bonus to the score and end the game    
        if timer.returnTick() == 5000:
        ...
        largeTurretShotSprites.update()
        largeTurretShotSprites.draw(screen)
